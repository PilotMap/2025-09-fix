...
def get_led_map_info():
    ...
    try:
        metar_elements = api_client.get_metars(valid_airports, 2.5, "xml")
        logger.info('Internet Available - METAR data retrieved')
        
        for metar in metar_elements:
            # Guard against missing station_id
            sid_el = metar.find('station_id')
            if sid_el is None or sid_el.text is None:
                continue
            stationId = sid_el.text

            # Guard against missing lat/lon
            lat_el = metar.find('latitude')
            lon_el = metar.find('longitude')
            ...
            lon_list.append(lon_f)

            # Guard against missing flight_category
            fc_el = metar.find('flight_category')
            if fc_el is None or fc_el.text is None:
                fl_cat = 'Not Reported'
            else:
                fl_cat = fc_el.text
            led_map_dict[stationId] = [lat_f,lon_f,fl_cat]
            
    except (NetworkError, APIError) as e:
        logger.warning('Error retrieving FAA data: ' + str(e))
        return
...
def get_apinfo():
    ...
    try:
        station_elements = api_client.get_station_info(valid_airports, "xml")
        logger.info('Internet Available - Station info retrieved')
        
        for station in station_elements:
            # Guard against missing station_id
            sid_el = station.find('station_id')
            if sid_el is None or sid_el.text is None:
                continue
            stationId = sid_el.text

            # Guard against missing site
            site_el = station.find('site')
            if site_el is None or site_el.text is None:
                continue
            site = site_el.text

            if stationId[0] != 'K':
                # Guard against missing country
                country_el = station.find('country')
