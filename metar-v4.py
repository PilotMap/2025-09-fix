...
try:
    ...
    class FAAAPIClient:
        """Dummy FAA API client for fallback"""
        def __init__(self, *args, **kwargs):
            pass
        
        def get_metars(self, *args, **kwargs):
            return []
        
        def get_tafs(self, *args, **kwargs):
            return []
...
logger.info(f"Initializing LED strip with {LED_COUNT} LEDs")
strip = LedStrip(LED_COUNT)
logger.info(f"LED strip initialized with {strip.number} pixels")

# Clear any test patterns that might be left from initialization
logger.info("Clearing any test patterns...")
...
def comp_time(taf_time):
    global current_zulu
    # Use ISO 8601 parser for consistent timestamp handling
    taf_dt = parse_iso8601(taf_time)
    current_dt = parse_iso8601(current_zulu)
    if taf_dt and current_dt:
        # Both are timezone-aware UTC datetimes, so comparison is safe
        diff = taf_dt - current_dt
...
while (outerloop):
    ...
    zulu = datetime.now(timezone.utc) + timedelta(hours=hour_to_display)     #Get current time plus Offset
    ...
    # Use centralized API client for METARs and TAFs
    if metar_taf_mos != 2 and metar_taf_mos != 3:
        # Check internet availability and get IP address
        ...
        valid_airports = [ap for ap in airports if ap not in ["NULL", "LGND"]]
        
        # Initialize API client only if available
        if USE_NEW_API_CLIENT:
            api_client = FAAAPIClient()
            
            try:
                if metar_taf_mos == 1:  # METAR data
                    metar_elements = api_client.get_metars(valid_airports, metar_age, "xml")
                    # Create root element from METAR elements
                    root = ET.Element('response')
                    for metar in metar_elements:
                        root.append(metar)
                    logger.info('Internet Available - METAR data retrieved')
                    
                elif metar_taf_mos == 0:  # TAF data
                    taf_elements = api_client.get_tafs(valid_airports, metar_age, "xml")
                    # Create root element from TAF elements
                    root = ET.Element('response')
                    for taf in taf_elements:
                        root.append(taf)
    ...
    if metar_taf_mos == 0: #0 equals display TAF. This routine will decode the TAF, pick the appropriate time frame to display.
        ...
        for taf in root.iter('TAF'):                            #iterate through each airport's TAF
            ...
            for forecast in taf.findall('forecast'):        #Now look at the forecasts for the airport
                ...
                # Parse timestamps using ISO 8601 parser
                tf = parse_iso8601(taf_time_from)
                tt = parse_iso8601(taf_time_to)
                ...
                if tf and tt and tf <= now_utc <= tt: #test if current time plus offset falls within taf's timeframe
