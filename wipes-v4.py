...
if __name__ == '__main__':
    ...
    # Build pindict dictionary and filter valid airports
    i = 0
    nullpins = []
    valid_airports = []
    ...
    for airportcode in airports:
        if airportcode == "NULL" or airportcode == "LGND":
            ...
            valid_airports.append(airportcode)
        i += 1

    # Use centralized API client for METAR data
    ...
    for metar in root.iter('METAR'):
        # Safely extract station_id
        station_id_elem = metar.find('station_id')
        if station_id_elem is None or station_id_elem.text is None:
            continue  # Skip this METAR if no station_id
        stationId = station_id_elem.text

        # Guard against missing lat/lon - skip if either is missing
        lat_elem = metar.find('latitude')
        lon_elem = metar.find('longitude')
        if lat_elem is None or lon_elem is None or lat_elem.text is None or lon_elem.text is None:
            continue  # Skip this METAR if lat/lon are missing

        try:
            # Convert to float for proper bounds calculation
            lat_f = float(lat_elem.text)
            lon_f = float(lon_elem.text)
        except (TypeError, ValueError):
            continue  # Skip this station if lat/lon are invalid

        # Only populate dictionaries if we have valid coordinates
        if stationId in latdict:
            print ("Duplicate, only saved the first weather")
        else:
            latdict[stationId] = lat_f            #build latitude dictionary with float

        if stationId in londict:
            print ("Duplicate, only saved the first weather")
        else:
            londict[stationId] = lon_f            #build longitude dictionary with float

        apinfodict[stationId]=[pindict[stationId],lat_f,lon_f] #Build Dictionary with structure:{airport id[pin num,lat,lon]}
    ...
    for key, value in latdict.items():
        latlist.append(value)  # value is already a float
    logger.debug(latlist)

    for key, value in londict.items():
        lonlist.append(value)  # value is already a float
